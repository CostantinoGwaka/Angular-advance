...
import {
  ...
  ItemsSortOption
} from "../../shared/components/items-sort-filter/items-sort-filter.component";
...
import {
  GET_SHAREABLE_DOCUMENT_CATEGORY_PUBLIC_DATA,
...
import {SharedDocumentsItemsComponent} from './shared-documents-items/shared-documents-items.component';
import {LoaderComponent} from '../../shared/components/loader/loader.component';
import { TitleCasePipe } from '@angular/common';
import {MatIconModule} from '@angular/material/icon';
import {
  PaginatedSelectComponent
} from '../../shared/components/dynamic-forms-components/paginated-select/paginated-select.component';
...
@Component({
  ...
  standalone: true,
  imports: [LayoutComponent, ParallaxContainerComponent, MatFormFieldModule, MatInputModule, FormsModule, PaginatedSelectComponent, MatIconModule, ItemsSortFilterComponent, LoaderComponent, SharedDocumentsItemsComponent, FrontendPaginatorComponent, TitleCasePipe, TranslatePipe]
})
export class SharedDocumentsComponent implements OnInit {
  ...
  subtitle: string = 'A list of all published documents';
  ...
  sortOptions: ItemsSortOption[] = [
    // {
    //   label: 'Document Title - Ascending',
    //   fieldName: 'title',
    //   orderDirection: 'ASC',
    // },
    // {
    //   label: 'Document Title - Descending',
    //   fieldName: 'title',
    //   orderDirection: 'DESC',
    // },
    {
  ...
  categorySearchField: FieldConfig = {
    type: FieldType.paginatedselect,
    query: GET_SHAREABLE_DOCUMENT_CATEGORY_PUBLIC_DATA,
  ...
  ngOnInit(): void {
    this.queryParamsSubscription = this.activeRoute.queryParams.subscribe(
      async (params) => {
        ...
        this.setQueryParamToFilterInputs(params, 'code', 'code');
        this.items = [];
  ...
  setQueryParamToFilterInputs(
    ...
    if (this.filters.code) {
      this.setMustHaveFilter('shareableDocumentCategoryCode', 'EQ', this.filters[inputName]);
    }
  }
  ...
  async clearSearch() {
    ...
    this.resetPagination();
    this.removeMustHaveFilter('shareableDocumentCategoryCode');
    this.getItems().then();
  ...
  async searchByInputFilters() {
    this.resetPagination();
    this.loading = true;
    this.subtitle = `A list of all published documents | filtered by ${this.filters.code ? this.filters.code : this.filters.search}`;
  ...
  onCategory(event: any) {
    const selectedCategory = { ...event.object };
    if (selectedCategory) {
      this.setInputFilter(
        'selectedCategory',
        'shareableDocumentCategoryCode',
        selectedCategory.code,
        `Search by Category "${selectedCategory.name}"`
      );
    }
  }
