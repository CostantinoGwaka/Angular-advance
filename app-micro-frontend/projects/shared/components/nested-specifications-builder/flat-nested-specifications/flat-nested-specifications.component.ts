...
import {
	EditableSpecsItemSizes,
	EditableSpecsItemComponent,
} from '../editable-specs-table/editable-specs-item/editable-specs-item.component';
import {
	BOQsCompletionStatusChangeEvent,
	FilteredItem,
	NestedSpecificationsInfoBarComponent,
} from '../nested-specifications-info-bar/nested-specifications-info-bar.component';
import {
...
} from '../specification-item-manager-dialog/specification-item-manager-dialog.component';
...
import { BoqSummaryComponent } from '../../boqs/boq-summary/boq-summary.component';
import {
	SpecificationDescriptionEditorComponent,
	SpecificationDescriptionItemManagerDialogResults,
} from '../specification-description-editor/specification-description-editor.component';
...
export interface SortingResults {
	success: boolean;
	message: string;
}

@Component({
	selector: 'app-flat-nested-specifications',
	templateUrl: './flat-nested-specifications.component.html',
	styleUrls: ['./flat-nested-specifications.component.scss'],
	standalone: true,
...
export class FlatNestedSpecificationsComponent implements OnInit {
	@Input()
	...
	needsExchangeRate: boolean = false;
	...
	psItemsOriginalCopy: FlatNestedSpecificationItemView[] = [];
	percentItemsOriginalCopy: FlatNestedSpecificationItemView[] = [];
	...
	exchangeRateChangeCount = 0;

	initialConversionDone = false;
	...
	ngOnInit(): void {
		this.boqsService.currentTenderDate.set(this.requisitionDate());
		setTimeout(() => {
			this.show = true;
		}, 500);
	...
	ngOnChanges(changes: SimpleChanges) {
		if (changes['exchangeRate']) {
			if (this.flatNestedSpecificationItemsView.length > 0) {
				this.reactToCurrencyChanges();
			}
		}
	}
	...
	reactToCurrencyChanges() {
		this.exchangeRateChangeCount++;
		if (this.psItemsOriginalCopy.length == 0) {
			this.psItemsOriginalCopy = JSON.parse(
				JSON.stringify(
		...
		if (this.percentItemsOriginalCopy.length == 0) {
			this.percentItemsOriginalCopy = JSON.parse(
				JSON.stringify(
					this.flatNestedSpecificationItemsView.filter(
						(item) =>
							item.editableSpecificationItem?.formula?.formulaName ==
		...
		for (let i = 0; i < this.flatNestedSpecificationItemsView.length; i++) {
			...
			if (
				...
				let itemCopy = this.psItemsOriginalCopy.find(
					(psItem) =>
						psItem.editableSpecificationItem.uuid ==
						item.editableSpecificationItem.uuid,
				);

				let formula = itemCopy.editableSpecificationItem?.formula;

				let unitRate =
					this.nestedSpecificationsService.getUnitRateFromPSFormula(formula);

				let convertedUnitRate = unitRate / this.exchangeRate;

				formula =
					this.nestedSpecificationsService.setPSFormula(convertedUnitRate);

				item = {
					...item,
					editableSpecificationItem: {
						...itemCopy.editableSpecificationItem,
						formula: formula,
						unitRate: convertedUnitRate,
						total:
							convertedUnitRate *
							parseFloat(itemCopy.editableSpecificationItem?.value) || 0,
					},
				};

				// let affectedItems =
				//   this.calculatePercentageValueForReferringItemsBySourceItemUuid(
				//     item.editableSpecificationItem,
				//     item.groupId
				//   );
				...
				//
				//   if (this.exchangeRateChangeCount == 1) {
				//     if (
				//       affectedItem.editableSpecificationItem.total != null &&
				//       this.checkIfItemQuantityOrUnitRateIsAffected(
				//         affectedItem.editableSpecificationItem,
				//         this.percentItemsOriginalCopy
				//       )
				//     ) {
				...
				//     }
				//   } else {
				//     if (affectedItem.editableSpecificationItem.total != null) {
				//
				//       this.nestedSpecificationsService.updateItemState(
				//         affectedItem.editableSpecificationItem,
				//         ItemsStateCodes.MODIFIED,
			...
			this.flatNestedSpecificationItemsView[i] = item;
		}
	}

	checkIfItemQuantityOrUnitRateIsAffected(
		item: EditableSpecificationItem,
		items: FlatNestedSpecificationItemView[],
	): boolean {
		return items.some((itemView) => {
			if (item.uuid === itemView.uuid && item.total !== null) {
				const isAffected =
					item.unitRate !== itemView.editableSpecificationItem.unitRate ||
					item.total !== itemView.editableSpecificationItem.total;
	...
	calculatePercentageValueForReferringItemsBySourceItemUuid(
		sourceItem: EditableSpecificationItem,
		groupId: number,
	) {
		let referringFields = this.flatNestedSpecificationItemsView.filter(
			(item) =>
				item?.editableSpecificationItem?.formula?.inputFields.filter(
					(field) => field.itemId == sourceItem.uuid,
				).length > 0 && item.groupId == groupId,
		);
		...
		return JSON.parse(JSON.stringify(referringFields));
	}
