...
export class ApproveTaxComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatBottomSheetRef<ApproveTaxComponent>,
  ...
  checkSection() {
    ...
    this.tendererEmbassyApprovalList.map((item) => {
      if (item.embassyTendererApprovalSection.includes("BUSINESS_REGISTRATION_DETAILS")) {
        this.comment = item;
        return true;
      } else {
        return false;
  ...
  async verify() {
    try {
      const response: any = await this.apollo.mutate({
        ...
        variables: {
          tendererEmbassyApprovalDto: {
            ...
            embassyTendererApprovalSection: "TAX_DETAILS",
      ...
      if (response.data.approveTendererDetailsAtEmbassy?.code !== 9000) {
        ...
        this.notificationService.successMessage(`Tax Details has been VERIFIED successfully`);
  ...
  async unVerify() {
    try {
      ...
      if (response.data.approveTendererDetailsAtEmbassy?.code !== 9000) {
        ...
        this.notificationService.successMessage(`Tax Details has been UN-VERIFIED successfully`);
        this.dialogRef.dismiss({ status: status, reason: this.reason });
        window.location.reload(); //reload after send
      }
    } catch (e) {
