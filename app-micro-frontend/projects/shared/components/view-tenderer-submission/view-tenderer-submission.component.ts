...
import {
  BidInterest,
  SubmissionCriteria,
} from '../../../modules/nest-tenderer/store/submission/submission.model';
...
import { GET_JVC_FOR_SPECIFIC_TENDER_SUBMISSION } from '../../../modules/nest-tenderer/store/joint-venture/joint-venture.graphql';
import { JointVenture } from '../../../modules/nest-tenderer/store/joint-venture/joint-venture.model';
...
@Component({
  selector: 'app-view-tenderer-submission',
  templateUrl: './view-tenderer-submission.component.html',
  styleUrls: ['./view-tenderer-submission.component.scss'],
  animations: [fadeIn],
  standalone: true,
  imports: [
    LoaderComponent,
...
export class ViewTendererSubmissionComponent implements OnInit, OnDestroy {
  @Input() submissionUuid: string;
  @Input() teamUuid: string;
  @Input() viewDetailsTitle: string;
  @Input() checkRequired: boolean = false;
  @Input() showTechnicalOnly: boolean = false;
  /** to be used when submission uuid is passed */
  @Input() automaticEvaluation: boolean = false;
  @Input() forZeroBid: boolean = false;
  /** to be used when boolean automatically is passed */
  @Input() updateBidPrice: boolean = false;
  @Input() updateBidPriceForConflictingTotals: boolean = false;
  /** to be used when submission uuid is passed */
  @Input() showEvaluationDetail: boolean = false;
  /** to be used when submission is viewed after evaluation */
  ...
  financialCriteriaFilter: string[] = [
    'PRICING',
    'PRICED_BOQ',
    'PRICED_SCHEDULE_OF_ACTIVITIES',
    'FINANCIAL_PROPOSAL',
  ];
  ...
  constructor(
    ...
    private settingServices: SettingsService // private tenderFormGeneratorService: TenderFormGeneratorService,
  ) { }
  ...
  //   // if (changes['submissionUuid'] && changes['submissionUuid'].currentValue) {
  //   //   this.generateTenderForm(changes['submissionUuid'].currentValue);
  //   // }
  // }
  ...
  // generateTenderForm(submissionUuid) {
  //   this.tenderFormGeneratorService$ = this.tenderFormGeneratorService.generateTenderForm(submissionUuid);
  //   this.subscriptions.push(
  //     this.tenderFormGeneratorService$.subscribe((change: TenderFormGeneratorChange) => {
  //
  //     })
  //   );
  // }
  ...
  fetchSubmission() {
    if (!this.submissionUuid) {
      this.getTendererBidInterest().then(async (_) => {
        if (this.checkRequired) {
          await this.isMySubmission(this.submission.uuid);
        }
        this.onCheckTenderJVC().then();
      });
    } else {
      this.getLotSubmission().then(async (_) => {
        await this.onCheckTenderJVC();
  ...
  setCriteriaGroupMap() {
    if (this.evaluationCriteria.length) {
      this.evaluationCriteria.sort(
        (a, b) => a.groupSortNumber - b.groupSortNumber
      );
    }
  }
    ...
    try {
      ...
      if (response.data.getSubmissionCriteriaBySubmissionUuid.code == 9000) {
        this.evaluationCriteria = [
          ...(response.data.getSubmissionCriteriaBySubmissionUuid.dataList ||
            []),
        ];
        this.evaluationCriteria.sort((a, b) => a.sortNumber - b.sortNumber);

        if (this.showTechnicalOnly) {
          this.evaluationCriteria = this.evaluationCriteria.filter(
            (criteria) =>
              !this.financialCriteriaFilter.includes(
                criteria.tenderEvaluationCriteria.tendererResponseType
              )
          );
        }
  ...
  async getSubmissionReport() {
    try {
      ...
      if (response.data.getTeamSummaryBySubmissionUuid.data) {
        ...
        if (this.tableHeaders.length !== 0) {
          this.tableHeaders.forEach((item, index) => {
            headerData.push({ name: 'header' + index, label: item });
          });
        }
      ...
      this.notificationService.errorMessage(
        'Problem Occurred while fetching tenderer evaluation details, Please  try again'
      );
    }
  }
  ...
  getTableRows(submissionCriteriaUuid: string) {
    return this.tableRows.filter(
      (item) => item.submissionCriteria.uuid === submissionCriteriaUuid
    );
  }
  ...
  async isMySubmission(submissionUuid: string) {
    if (!submissionUuid) {
      this.notificationService.errorMessage(
        'Important data is missing, Please try again.'
      );
      return;
    }
    this.loading = true;
    try {
      ...
      if (response.data.isMySubmission.code == 9000) {
        if (!response.data.isMySubmission.data) {
          this.notificationService.errorMessage(
            'Access denied, you are not allowed to view this bid details'
          );
  ...
  async showTenderForm() {
    ...
    data = await this.attachmentService.getSignedAttachment(
      this.submission.signedTenderFormUuid
    );

    if (!data) {
      data = await this.attachmentService.getAttachment(
        this.submission.signedTenderFormUuid
      );
    }
  ...
  async onCheckTenderJVC() {
    this.loading = true;

    try {
      const response: any = await this.apollo.fetchData({
        query: GET_JVC_FOR_SPECIFIC_TENDER_SUBMISSION,
        apolloNamespace: ApolloNamespace.uaa,
        variables: {
          tendererUuid: this.submission?.tendererUuid,
          entityUuid: this.submission?.mainEntityUuid,
          entityType: this.submission?.entityType,
        },
      });

      if (response.data.getJointVentureByEntityUuidAndTypeData.code == 9000) {
        const values =
          response.data.getJointVentureByEntityUuidAndTypeData.data;
        if (values) {
          this.jointVenture = values;
        }
      }
      this.loading = false;
    } catch (e) {
  ...
  async viewAttachment(attachment: string) {
    if (attachment) {
      this.fetchingAttachment = true;
      const data = await this.attachmentService.getAttachment(attachment);
      this.settingServices.viewFile(data, 'pdf').then(() => {
